...
Os resultados da simulação podem ser vistos na figura \ref{fig:simLoop01}.
\begin{figure}[H]
...
\begin{par}
	\textbf{Q}: Por que no item 1.5 acima, os torques finais obtidos são nulos? O torque final não deveria ser igual ao torque de compensação da força da gravidade?
\end{par}

\vspace{0.6cm}

\begin{par}
	\textbf{R}: No item 1.5 a gravidade foi desconsiderada e, por isso, o torque final obtido é nulo, já que no estado final não há gravidade a ser compensada.
\end{par}

\subsection*{Questão 2}

\begin{par}
	\textbf{Q}: No item 1.4 foi indicado um comando Matlab para se obter a matriz de massa/inércia no espaço das juntas $M(q)$. Como você faria para obter essa matriz de massa utilizando  o método de Newton-Euler além de manipulação simbólica? Veja os apêndices B e C da mesma tese acima. Essa matriz corresponde a matriz $M(q)$ da equação geral da dinâmica de um robô, como dado abaixo, onde $\tau$ é o vetor de torques nas juntas.
\end{par}
...
\begin{par}
	\textbf{R}: Usando o método de Newton-Euler, é possível obter $\tau$ em função dos parâmetros cinéticos do robô:
\end{par}
...
\begin{par}
	Com isso, fixando um valor $q$ qualquer, fazendo $\dot{q} = \vec{0}$, $\ddot{q} = \left[\begin{matrix}1 & 0 & \dots & 0 \end{matrix}\right]^T$ e substituindo a expresão de $\tau$ na equação geral da dinâmica, chega-se em:
\end{par}
...
\begin{par}
com $\tau_i$ e $g_i$ conhecidos. Desse modo, pode-se obter os valores de $M_{i1}$. Usando o mesmo processo para $\ddot{q} = \left[\begin{matrix}0 & 1 & 0 & \dots & 0 \end{matrix}\right]^T$, obtém-se os valores de $M_{i2}$. Assim, repetindo o processo para todas as $N$ colunas, consegue-se deduzir todos os elementos da matriz $M_{N \times N}$.
\end{par}

\subsection*{Questão 3}

\begin{par}
	\textbf{Q}: Por que é que essa matriz de massa $M(q)$ não é do tipo da matriz de massa abaixo de um corpo rígido, que é uma matriz 6x6 com uma matriz diagonal $M$ e outra matriz de inércia do corpo $J$?
\end{par}
...
\begin{par}
	\textbf{R}: Como o robô não se trata de um corpo rígido, mas sim de um corpo com várias partes móveis ligadas através de juntas, a matriz $M$ não apresenta o mesmo formato observado no caso de corpos rígidos.
	Os elementos $M_{ij}$ fora da diagonal representam as inércias de acoplamento, que relacionam a aceleração na junta $i$ com o torque na junta $j$.
\end{par}
